...
const columns = [
  ...
  { id: 'In Progress', label: 'In Progress', color: 'border-teal-300', bg: 'bg-teal-50' },
...
export default function ProjectsPage() {
  ...
  if (loading) return <div className="flex justify-center py-12"><svg className="w-8 h-8 animate-spin text-emerald-600" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg></div>;
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={creating} className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-medium hover:bg-emerald-700 disabled:opacity-60">{creating ? 'Creating...' : 'Create'}</button>
              </div>
            </form>
          </div>
        </div>
